        ...
        <template v-else>
          ...
          >
            <v-list-item-action v-if="item.icon">
              <i
                :class="
                  !item.color && $route.path == item.route
                    ? `primary--text iconfont icon-menu ${item.icon}`
                    : `iconfont icon-menu ${item.icon}`
                "
              ></i>
            </v-list-item-action>
            <v-list-item-action v-if="item.img">
              <v-img :src="item.img" :width="item.size || 25"></v-img>
            </v-list-item-action>
            <v-list-item-content>
              <v-list-item-title
                :class="
              ...
              >
                <v-badge
                  v-if="
                    ...
                    !$_.isEmpty(typeUnreadNotifications)
                  "
                  color="red"
                  dot
                  >{{ item.text }}</v-badge
